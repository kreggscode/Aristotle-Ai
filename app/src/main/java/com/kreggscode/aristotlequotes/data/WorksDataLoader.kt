...
class WorksDataLoader(private val context: Context) {
    ...
    suspend fun loadMajorWorks(): List<MajorWork> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("aristotle_major_works.json")
                .bufferedReader()
                .use { it.readText() }
    ...
    suspend fun loadEssays(): List<Essay> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("aristotle_essays.json")
                .bufferedReader()
                .use { it.readText() }
    ...
    suspend fun loadLetters(): List<Letter> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("aristotle_letters.json")
                .bufferedReader()
                .use { it.readText() }
    ...
    suspend fun loadPapers(): List<Paper> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("aristotle_papers.json")
                .bufferedReader()
                .use { it.readText() }
